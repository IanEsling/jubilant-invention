...
class GroceryOperationsTest {
    ...
    @Test
    void listItems() {
        Collection<Item> listedItems = GroceryOperations.forItems(items).listItems();
        assertThat(listedItems).containsExactlyInAnyOrderElementsOf(items.getItems());
    }

    @Test
    void throwIfNoItemsToBeSupplied() {
        assertThrows(UnsupportedOperationException.class, () -> GroceryOperations.forItems(null));
        assertThrows(UnsupportedOperationException.class, () -> GroceryOperations.forItems(Items.of()));
    }

    @Test
    void addItemsToBasket() {
        GroceryOperations testee = GroceryOperations.forItems(items);
    ...
    void calculatePriceOfBasketWithSingleItem() {
        ...
        GroceryOperations testee = GroceryOperations.forItems(Items.of(Item.item("Single", x2cost)));
    ...
    void calculatePriceOfBasketWithMultipleItems() {
        ...
        GroceryOperations testee = GroceryOperations.forItems(Items.of(Item.item(times2, x2cost),
    ...
    void applySpecialOffer() {
        ...
        var withoutOffer = GroceryOperations.forItems(items);
        var withOffer = GroceryOperations.forItemsAndOffers(items, offer);
    ...
    void applyMultipleSpecialOffer() {
        ...
        var withoutOffer = GroceryOperations.forItems(items);
        var withOffer = GroceryOperations.forItemsAndOffers(items, offer1, offer2, offer3);
        List.of(withOffer, withoutOffer).forEach(ce -> {
            ce.addToBasket(2, item1.getName());
            ce.addToBasket(3, item2.getName());
            ce.addToBasket(4, item3.getName());
